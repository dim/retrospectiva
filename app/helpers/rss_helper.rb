module RssHelper
  ...
  protected
    ...
    def subscribe_via_aol(title, url)
      subscribe_via :aol, "http://favorites.my.aol.com/ffclient/AddFeed?url=#{CGI.escape(url)}", "Add to myAOL"
    end

    def subscribe_via_bloglines(title, url)
      subscribe_via :bloglines, "http://www.bloglines.com/sub/#{CGI.escape(url)}", "Add to Bloglines"
    end

    def subscribe_via_google(title, url)
      subscribe_via :google, "http://fusion.google.com/add?source=atgs&feedurl=#{CGI.escape(url)}", "Add to Google"
    end

    def subscribe_via_newsgator(title, url)
      subscribe_via :newsgator, "http://www.newsgator.com/ngs/subscriber/subext.aspx?url=#{CGI.escape(url)}", "Subscribe in NewsGator Online"
    end

    def  subscribe_via_technorati(title, url)
      subscribe_via :technorati, "http://technorati.com/faves?add=#{CGI.escape(url)}", "Add to Technorati"
    end

    def subscribe_via_yahoo(title, url)
      subscribe_via :yahoo, "http://us.rd.yahoo.com/my/atm/#{CGI.escape(site_name)}/#{CGI.escape(title)}/*http://add.my.yahoo.com/rss?url=#{CGI.escape(url)}", "Add to My Yahoo!"
    end
