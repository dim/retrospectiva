module Grit
  ...
  class Repo
    ...
    # An array of SHA-1 hashed representing the history of a given ref/commit    
    def rev_list(commit = 'HEAD', *paths)
      options = paths.last.is_a?(Hash) ? paths.pop : {}      
      args = paths.any? ? [commit, '--', *paths] : [commit]
      self.git.rev_list(options, *args).split
    end
    ...
    def log(commit = 'master', path = nil, options = {})
      ...
      result = self.git.rev_list(actual_options, *arg)
      Commit.list_from_string(self, result)
    end
