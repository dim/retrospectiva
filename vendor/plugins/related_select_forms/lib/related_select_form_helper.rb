...
module ActionView::Helpers::RelatedSelectFormHelper
  ...
  def related_collection_select(object_name, method, parent_element, collection, value_method, text_method, reference_method, options = {}, html_options = {})                  
    relations = collection.inject({}) do |result, record|
      ...
      result[reference_value] ||= ActiveSupport::OrderedHash.new
      #result[reference_value][record_value] ||= []
      result[reference_value][record_value] = record.send(text_method)  
      result
    end
    parent_tag_id = parent_element.is_a?(Array) ? parent_element.map(&:to_s).join('_') : parent_element.to_s
...
class ActionView::Helpers::InstanceTag #:nodoc:  
  ...
  def to_related_collection_select_tag(parent_tag_id, relations, options = {}, html_options = {})  #:nodoc: 
    ...
    (related_select_cache.size == 1 ? @template_object.javascript_tag(javascript_code_for_related_select_extension) : '') +
      @template_object.content_tag("select", '', html_options) +
      @template_object.javascript_tag(javascript_code_for_relation_hookup(parent_tag_id, relations, selected_value, options)) + 
      @template_object.javascript_tag(javascript_code_for_preselection(selected_value))
  end  
  ...
  private
    ...
    def javascript_code_for_relation_hash(relations, selected, options)
      relations.map do |reference_value, records|           
        choices = []
        choices << "new Option('', '')" if options[:include_blank]
        if selected.blank? && options[:prompt]
          choices << "new Option(#{options[:prompt].kind_of?(String) ? options[:prompt] : 'Please select'}, '')"
        end

        choices += records.map do |value, text|
          text = @template_object.truncate(text, :length => options[:truncate]) if options[:truncate] 
          "new Option(#{text.to_json}, #{value.to_json})" 
        end

        "#{reference_value.to_json}: [#{choices.join(', ')}]"
      end.join(",\n        ")
    end      
