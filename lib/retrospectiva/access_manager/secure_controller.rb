      ...
      module ClassMethods
        ...
        def require_permissions(resource = nil, options = {})
          ...
          unless resource.blank?
            permission_map = options.inject({}) do |result, (permission, actions)|
              [actions].flatten.map(&:to_s).each do |action| 
                result[action] ||= []
                result[action] << [resource.to_sym, permission.to_sym]
              end
              result
            end
            write_authorization_attribute(:require_permissions, permission_map) 
          end
          read_authorization_attribute(:require_permissions, {})
        end
        ...
        def require_user(*actions)
          ensure_authorize_before_filter
          write_authorization_attribute(:require_user, actions.map(&:to_s)) unless actions.blank?
          read_authorization_attribute(:require_user, [])
        end
        ...
        private    
          def write_authorization_attribute(attribute, value)
            write_inheritable_attribute("retro-am::authorization::#{attribute}", value)
          end

          def read_authorization_attribute(attribute, default = [])
            read_inheritable_attribute("retro-am::authorization::#{attribute}") || default
          end
            
          def ensure_authorize_before_filter
            unless authorized_controller?
              raise Error, "Unable to find an 'authorize' before_filter."
            end
          end
      end
