...
begin
  ...
  namespace :spec do
    ...
    desc "Run the specs under extension"
    Spec::Rake::SpecTask.new(:extension => spec_prereq) do |t|
      name = (ENV['RETRO_EXT'] || ENV['X'])
      t.spec_opts = ['--options', "\"#{RAILS_ROOT}/spec/spec.opts\""]
      t.spec_files = FileList["extensions/#{name}/**/*_spec.rb"]
    end
